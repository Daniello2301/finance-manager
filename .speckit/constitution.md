...
| 2026-07-16 | **Se añade Google como proveedor de acceso, junto al correo y contraseña — no en su lugar.** `passwordHash` deja de ser obligatorio y `User` gana `googleId`. La identidad sigue siendo **nuestro `User._id`**: el `sub` de Google nunca sale del callback. Entrar con Google **siembra las 21 categorías** si el usuario es nuevo (FR-015), **vincula** a la cuenta existente si el correo coincide (FR-016) y **solo** si Google lo reporta verificado (FR-017); una cuenta de Google no puede entrar con contraseña (FR-018). | informatica@fabricato.com | Ratificado porque toca un módulo ya ratificado y el modelo de identidad, que es justo lo que el Amendment Process exige no cambiar en un diff silencioso. Cabe como enmienda —y no como módulo nuevo— precisamente porque **el Principio 8 no se toca**: todo sigue colgando de nuestro ObjectId. Ése fue el motivo exacto por el que se rechazó Clerk (2026-07-13), donde la identidad *no* era nuestro ObjectId y la siembra de categorías pasaba a depender de un webhook `user.created`. Con NextAuth la siembra ocurre dentro del propio flujo de acceso, sin modo de fallo nuevo. **La regla que sostiene todo es FR-017**: vincular por correo sin exigir que Google lo haya verificado sería regalar la cuenta a quien registre ese correo — apropiación, no un detalle. Y FR-018 devuelve `null`, no un mensaje propio: decir "esta cuenta usa Google" delataría qué correos existen y rompería FR-014. |
| 2026-07-15 | **La generación automática de un recurrente es un hecho consumado y se registra aunque deje la cuenta en descubierto.** La spec de Recurrentes (FR-012) decía "pasa `confirmOverdraft: true`", pero ese mecanismo fue derogado el 2026-07-14. Se reemplaza por una bandera de servicio `allowOverdraft`, que **solo** activa el generador automático (`autoGenerate: true`) y que **ninguna ruta HTTP expone ni parsea de un body**. Un recurrente **manual** que sobregira NO la usa: pasa por la creación normal y recibe el diálogo interactivo de las cuatro salidas, como cualquier gasto. | informatica@fabricato.com | Coherente con la distinción ya ratificada el 2026-07-14: una *decisión* se frena, un *hecho consumado* se anota y marca la cuenta en descubierto. Un cargo domiciliado que ya venció es un hecho consumado por definición ("se cobra solo") — el banco ya lo movió, y negarse a anotarlo solo haría que la app mostrara un saldo *más* falso que el real (US2: "que mi saldo sea real cuando abro la app"). La "marca en descubierto" no es un campo nuevo: es el saldo negativo derivado que `isOverdrawn()`/`availableBalance()` ya muestran. La bandera es un argumento interno del servicio, no un `confirmOverdraft` reabierto para el usuario: la puerta que se cerró el 2026-07-14 era la de forzar una *decisión*, y esta no lo es. |
